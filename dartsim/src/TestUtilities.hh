...
namespace test
{
  class UnimplementedFeature : public virtual physics::Feature
  {
    ...
    public: UnimplementedFeature()
    {
      std::cerr << "Creating UnimplementedFeature\n";
    }

    public: virtual ~UnimplementedFeature()
    {
      std::cerr << "Destroying UnimplementedFeature\n";
    }
  };
}
...
void PrimeTheLoader(plugin::Loader &_loader)
{
       ...
       : physics::test::g_PhysicsPluginLibraries)
  {
    if (!library.empty())
    {
      auto loadedLibs = _loader.LoadLib(library, false);
      std::cerr << "Loading library: " << library << "\n";
